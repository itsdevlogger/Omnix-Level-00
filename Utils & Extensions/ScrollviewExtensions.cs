...
public static class ScrollViewFocusFunctions
{
    ...
    #if HAS_DO_TWEEN
    public static void FocusOnItem(this ScrollRect scrollView, RectTransform item, float scrollSpeed, Ease ease = Ease.Linear)
    {
        Vector2 finalPos = scrollView.CalculateFocusedScrollPosition(item);
        float duration = Vector2.Distance(finalPos, scrollView.normalizedPosition) / scrollSpeed;
        DOTween.To(
            getter: () => scrollView.normalizedPosition,
            setter: p => scrollView.normalizedPosition = p,
            finalPos,
            duration).SetEase(ease);
    }

    public static void FocusAtPoint(this ScrollRect scrollView, Vector2 focusPoint, float scrollSpeed, Ease ease = Ease.Linear)
    {
        Vector2 finalPos = scrollView.CalculateFocusedScrollPosition(focusPoint);
        float duration = Vector2.Distance(finalPos, scrollView.normalizedPosition) / scrollSpeed;
        DOTween.To(
            getter: () => scrollView.normalizedPosition,
            setter: p => scrollView.normalizedPosition = p,
